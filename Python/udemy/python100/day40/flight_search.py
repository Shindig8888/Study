...
class FlightSearch:
    ...
    def search(self, flight_to_list, max_price_list):
        ...
        for number in range(0, len(flight_to_list)):
            ...
            response = requests.get(url=self.URL, params=parameter1, headers=headers)
            flight_data.append(response.json())
            if len(flight_data[number]['data']) ==0:
                del flight_data[number]
                response = requests.get(url=self.URL, params=parameter2, headers=headers)
                flight_data.append(response.json())
        return flight_data
