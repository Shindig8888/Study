...
fn main() {
    ...
    let multi_num: Vec<_> = numbers.iter_mut().map(|x: &mut i32| (*x) * 3).collect();
    println!("{:?}", multi_num); // should print [3, 6, 9, 12]
    ...
    let mut pairs2: Vec<(i32, i32)> = vec![(0, 1), (2, 3), (4, 5)];
    for t in pairs2.iter_mut() {
        t.0 += 1;
    }
    println!("{:?}", pairs2)
}
